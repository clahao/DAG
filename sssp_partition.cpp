...
void sssp_priority(long long int &cal_times, int node_index, const CSR& csr, const CSR& csc, set<int> start, vector<int> node_map, vector<int> node_inv_map, vector<int> &global_dist, int start_id, int num_node ,int num){
//  int n = csr.n;
    int n = num_node;
    int partition = (n+num-1) / num;
    if(start.size() == 0)
    ...
    vector<int> dist(global_dist.begin()+start_id,global_dist.begin()+start_id+num_node);
    vector<int> dist_old(global_dist.begin()+start_id,global_dist.begin()+start_id+num_node);
    priority_queue<pair<int,double>,vector<pair<int,double>>,cmp_priority> priorityQueue;
    int part_index = node_index / num;  //需要进行块内迭代的块的数量
...
//        if(bm_part.get(part) == 0){
            bm_part.set(part);
//            vector<int> vertexs = vertex_n_partion(part, n, num);
//            vector<int> active_outdegree;
//            vector<double> active_value;
//            double outdegree_sum = 0;
//            for(int j = 0; j < vertexs.size(); j ++){
//                int u = vertexs[j];
//                if(bm.get(u)){
//                    u = node_inv_map[u];
//                    vector<int> neigh = csr.neighbors(u);
//                    active_value.push_back(dist[u]);
//                    active_outdegree.push_back(neigh.size());
//                    outdegree_sum += neigh.size();
//                }
//            }
//            double priority = 0;
//            for(int j = 0; j < active_value.size(); j ++){
//                priority += active_value[j] * active_outdegree[j] / outdegree_sum;
//            }
            vector<int> neigh = csr.neighbors(i-start_id);
            priorityQueue.push(make_pair(part,double(global_dist[i])/(log10(neigh.size()+1))));
//            priorityQueue.push(make_pair(part,priority));
//        }
...
//                    priorityQueue.push(make_pair(part_nu,dist[nu]));
                    bm.set(node_map[nu]);
//                    vector<int> vertexs_ = vertex_n_partion(part_nu, n, num);
//                    vector<int> active_outdegree;
//                    vector<double> active_value;
//                    double outdegree_sum = 0;
...
//                    }
                    ...
                    priorityQueue.push(make_pair(part_nu,double(dist[nu])/(log10(neigh.size()+1))));
                }
            }
        }
    }
