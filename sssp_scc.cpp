...
//    pre_runtime = pre_timer.Finish();
    ...
    for(int i = 0; i < scc_num; i ++){
        ...
        if(scc_node_num[i] > 1){
            ...
            set<int> row_set;
            set<int> col_set;
//        col_set.insert(source_node);
//        col_visit[source_node] = true;
//        col_inv_map[col] = source_node;
//        col_map[source_node] = col++;
//        while(row < csr.n || col < csr.n){
//            if(col < csr.n && col_set.empty()){
//                for(int j = 0; j < csr.n; j ++){
//                    if(!col_visit[j]){
//                        col_set.insert(j);
//                        col_visit[j] = true;
...
//                }
//            }
//            for(int node : col_set) {
//                for(int j = csr.row_ptr[node]; j < csr.row_ptr[node+1]; j ++){
//                    int neighbor_node = csr.col_idx[j];
//                    if(!row_visit[neighbor_node]){
//                        row_visit[neighbor_node] = true;
//                        row_set.insert(neighbor_node);
//                        row_inv_map[row] = neighbor_node;
//                        row_map[neighbor_node] = row++;
//                    }
//                }
//            }
//            col_set.clear();
//            if(row < csr.n && row_set.empty()){
//                for(int j = 0; j < csr.n; j ++){
//                    if(!row_visit[j]){
//                        row_set.insert(j);
//                        row_visit[j] = true;
...
//                }
//            }
//            for(int node : row_set) {
//                for(int j = csc.row_ptr[node]; j < csc.row_ptr[node+1]; j++){
//                    int sourceNode = csc.col_idx[j];
//                    if(!col_visit[sourceNode]){
//                        col_visit[sourceNode] = true;
//                        col_set.insert(sourceNode);
//                        col_inv_map[col] = sourceNode;
//                        col_map[sourceNode] = col++;
//                    }
//                }
//            }
//            row_set.clear();
//        }
//        vector<bool> flag(csr.n,false);
//        vector<bool> flag1(csr.n,false);
//        for(int j = 0; j < csr.n; j ++){
//            flag[col_map[j]] = true;
//            flag1[row_map[j]] = true;
//        }
//        for(int j = 0; j < csr.n; j ++){
//            assert(flag[j] == true);
//            assert(flag1[j] == true);
//        }
            ...
            //rabit reorder
//        vector<int> node_map(csr.n);
//        vector<int> node_inv_map(csr.n);
...
//            sssp_rabbit_push_pull_row(cal_times,cal_times_community,cal_times_between_communities,csr,csc,start_node_list[i],node_degree_map,node_degree_inv_map,community,global_dist,node_start_index,scc_node_num[i],num);
