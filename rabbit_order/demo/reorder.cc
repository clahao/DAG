...
vector<vector<int>> reorder_community(CSR csr){
    int num_nods = csr.n;
    std::vector<edge_list::aux::edge> edges;
    for(int i = 0; i < num_nods; i ++){
        for(int j = csr.row_ptr[i]; j < csr.row_ptr[i+1]; j ++){
            int neighbor_id = csr.col_idx[j];
            std::tuple<vint, vint, float> temp(i,neighbor_id,1);
    ...
    adjacency_list adj =  make_adj_list(n, edges);

    const auto g = rabbit_order::aggregate(adj);
    auto g1 = rabbit_order::aggregate(adj);
    auto p = rabbit_order::compute_perm(g);
//    std::copy(&p[0], &p[g.n()], std::ostream_iterator<vint>(std::cout, "\n"));

    int node_num = g.n();
    auto map = std::vector<int>(g.n(),0);
    auto inv_map = std::vector<int>(g.n(),0);
    auto c = std::vector<int>(g.n(),0);
    #pragma omp parallel for
    for (vint v = 0; v < g.n(); ++v){
        map[v] = p[v];
        inv_map[map[v]] = v;
        c[v] = rabbit_order::trace_com(v, &g1);
    }
    vector<vector<int>> result;
    result.push_back(map);
    result.push_back(inv_map);
    result.push_back(c);
...
////  std::cout<<"community: "<<endl;
////  detect_community(adj);
//  std::cout<<"reorder: "<<endl;
//  reorder(std::move(adj));
//
//  return EXIT_SUCCESS;
//}
